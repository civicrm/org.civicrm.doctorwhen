...
class CRM_DoctorWhen_Cleanups_ActivityCreated extends CRM_DoctorWhen_Cleanups_Base {
  ...
  public function getTitle() {
    return ts('"civicrm_activity.created_date" - Fill in missing values using "civicrm_log" (CRM-20958)');
  }
  ...
  public function enqueue(CRM_Queue_Queue $queue, $options) {
    ...
    for ($startId = $minId; $startId <= $maxId; $startId += self::DEFAULT_BATCH_SIZE) {
      ...
      $title = sprintf('CRM-20958 - Fill in missing values of "civicrm_activity.created_date" using civicrm_log (%d => %d)', $startId, $endId);
