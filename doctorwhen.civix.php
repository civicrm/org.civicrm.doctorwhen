...
function _doctorwhen_civix_civicrm_managed(&$entities) {
  ...
  foreach ($mgdFiles as $file) {
    ...
    foreach ($es as $e) {
      if (empty($e['module'])) {
        $e['module'] = 'org.civicrm.doctorwhen';
      }
      $entities[] = $e;
...
function _doctorwhen_civix_civicrm_angularModules(&$angularModules) {
  ...
  foreach ($files as $file) {
    ...
    $module = include $file;
    if (empty($module['ext'])) {
      $module['ext'] = 'org.civicrm.doctorwhen';
    }
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _doctorwhen_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item, $key);
      }
    }
    return $found;
  }
