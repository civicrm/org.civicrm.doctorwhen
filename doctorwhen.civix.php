 ...
 * extension.
 */
class CRM_DoctorWhen_ExtensionUtil {
  const SHORT_NAME = 'doctorwhen';
  const LONG_NAME = 'org.civicrm.doctorwhen';
  const CLASS_PREFIX = 'CRM_DoctorWhen';
...
use CRM_DoctorWhen_ExtensionUtil as E;
...
function _doctorwhen_civix_find_files($dir, $pattern) {
  ...
  while (!empty($todos)) {
    ...
    foreach (_doctorwhen_civix_glob("$subdir/$pattern") as $match) {
      if (!is_dir($match)) {
        $result[] = $match;
      }
    }
 ...
 * @return bool
 */
function _doctorwhen_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _doctorwhen_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _doctorwhen_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _doctorwhen_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _doctorwhen_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _doctorwhen_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _doctorwhen_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
